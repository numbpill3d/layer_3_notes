...
async function loadFile(filename) {
    try {
        ...
        if (fileData.encrypted) {
            const decryptKeyInput = document.getElementById('decryption-key');
            const decryptionKey = decryptKeyInput ? decryptKeyInput.value : '';
...
function filterByTag(tag) {
    try {
        ...
        if (Object.keys(filteredFiles).length === 0) {
            ...
            Object.entries(filteredFiles).forEach(([filename]) => {
